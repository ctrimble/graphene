...
void expand_comb_nr( int uniqueElements, int subsetSize, long combIndex, int outOffset, global int* out); 

kernel void expand_comb_nr_range( global RangeCombinatoricContext* context, global int* elements ) {
	int i = get_global_id(0);
	
	if( i < context->range.end - context->range.start ) {
	  long index = i + context->range.start;
	  expand_comb_nr( context->uniqueElements, context->subsetSize, index, i*(context->subsetSize), elements );
	}
}
 ...
 */
kernel void expand_comb_nr_table( global CombinatoricContext* context, global long* indices, global int* elements)
{
	int i = get_global_id(0);
...
void expand_comb_nr( int uniqueElements, int subsetSize, long combIndex, int outOffset, global int* out) {
	long index = combIndex;
	  ...
	  for( int e = 0; e < subsetSize; e++ ) {
	    ...
	    out[outOffset+e] = uniqueElements-n;
	    k--;
	    n--;
	  }
}
